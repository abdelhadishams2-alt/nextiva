/**
 * Astro Native Adapter — Full Astro page generation.
 *
 * Generates Astro pages with:
 * - .astro files with frontmatter metadata
...
const description = 'Astro with frontmatter metadata, island architecture, scoped CSS';
const extensions = ['.astro'];

/**
 * Generate a complete Astro article page.
 *
...
function generate(ir) {
  ...
  // 1. Main page (.astro)
  const pageContent = generateAstroPage({
    title, articleDesc, content, langConfig, tokens,
    images, sections, useTailwind, dir, lang, meta, faq,
  });
  files.push({ path: `src/pages/articles/${slug}.astro`, content: pageContent });

  // 2. Edit overlay island component (interactive — client:visible)
  const editOverlay = generateEditOverlayIsland();
  files.push({ path: 'src/components/EditOverlay.astro', content: editOverlay });
...
function generateAstroPage(opts) {
  const {
    title, articleDesc, content, langConfig, tokens,
    images, sections, useTailwind, dir, lang, meta, faq,
  } = opts;
  ...
  // Frontmatter block
  lines.push('---');
  lines.push("import EditOverlay from '../../components/EditOverlay.astro';");
  lines.push('');
  lines.push(`const title = ${JSON.stringify(title)};`);
  lines.push(`const description = ${JSON.stringify(articleDesc || title)};`);
  if (meta.author) {
    lines.push(`const author = ${JSON.stringify(meta.author)};`);
  }
  if (meta.publishedAt || meta.date) {
    lines.push(`const publishedAt = ${JSON.stringify(meta.publishedAt || meta.date)};`);
  }
  lines.push('---');
  lines.push('');

  // HTML head metadata
  lines.push('<html lang="' + escapeAttr(lang) + '" dir="' + escapeAttr(dir) + '">');
  lines.push('<head>');
  ...
  // Article
  lines.push(`  <article class="${cn('max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12', 'container')}">`);

  // Header with hero image
  lines.push(`    <header class="${cn('mb-12', 'header')}">`);
  lines.push(`      <h1 class="${cn('text-4xl font-bold tracking-tight mb-4', 'title')}">{title}</h1>`);
  if (articleDesc) {
    lines.push(`      <p class="${cn('text-lg text-muted-foreground', 'description')}">{description}</p>`);
  }
  if (heroHTML) {
    lines.push(heroHTML);
  }
  ...
  // Edit overlay — island architecture (client:visible)
...
function buildSectionsHTML(sections, images, useTailwind, fallbackContent) {
  if ((!sections || sections.length === 0) && fallbackContent) {
    return indent(fallbackContent, 8);
  }
  ...
  for (const section of sections) {
    ...
    if (sectionType === 'code' || section.codeBlock) {
      ...
      parts.push(buildInlineImageHTML(section, useTailwind));
    } else if (sectionContent) {
      parts.push(indent(sectionContent, 8));
    }
  }
...
function buildInlineImageHTML(section, useTailwind) {
...
// ── Edit Overlay Island Component ──────────────────────────────────

function generateEditOverlayIsland() {
  return `---
// EditOverlay — interactive island component for article editing.
// Uses client-side JavaScript for interactivity (island architecture).
---

<div id="edit-overlay-root">
  <button id="edit-toggle" class="edit-button" aria-label="Edit article">
    Edit
  </button>
</div>
...
<script>
  ...
  const root = document.getElementById('edit-overlay-root');
  const bridgeUrl = 'http://127.0.0.1:19847';

  toggle?.addEventListener('click', () => {
    const panel = document.createElement('div');
    panel.className = 'overlay-backdrop';
    panel.innerHTML = \`
